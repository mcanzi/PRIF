...
suicide_rate ~ alcohol consumption
...
```{r, eval = F, warning = F}
df %>% # data
  ggplot(aes(x = alcohol_consumption_per_capita, 
             y = suicide_mortality_rate)) + # aesthetics
  geom_point() + 
  geom_smooth(method="lm") + 
  theme(legend.position="bottom") # geometry+
```
...
### Run this
df %>% filter(country == "AUS", sex == "TOT") -> ex.a2
#df %>% filter(country %in% countries) -> ex.a2.alt
```
...
df %>% select(1:10)
df %>% select(1, 10, 2:9)
df %>% select(index = 1, 10, 2:9)
df %>% select(-1, -2, -3)
```

```{r}
ex.a2 %>% ggplot(aes(x = time, y = government_health_expenditure_pct_gdp)) + 
  geom_line() + ylim(4, 8) +
  geom_point()
```
...
```{r}
ex.a2 %>% ggplot(aes(x = time, y = government_health_expenditure_pct_gdp)) + 
  geom_smooth() + ylim(4, 8) + geom_point()
```
...
df %>% 
  #filter(sex != "TOT") %>% 
  #mutate(sex = if_else(sex == "BOY", "Male", "Female")) %>% 
  #mutate(sex = sub("BOY", "Male", sex)) %>% 
  mutate(sex = case_when(
    sex == "BOY" ~ "Male",
    sex == "GIRL" ~ "Female", 
    sex == "TOT" ~ "Total")) %>% 
  filter(sex != "Total") %>% 
  mutate(sex = fct_relevel(as.factor(sex), "Male")) %>% 
...
### Write your code here 
df %>% 
  filter(country == "DEU" | country == "ITA") %>% 
  ...
  scale_color_manual(values = c("indianred2", "deepskyblue4")) 
```
...
```{r}
df %>% filter((country == "ITA" | country == "DEU") & sex != "TOT") %>% 
  ggplot(aes(x = time, y = rating, col = sex, pch = sex, linetype = country)) + 
  geom_point() + geom_line() + 
  scale_color_manual(values = c("indianred3", "plum"), 
                     name = "Sex", 
                     labels = c("Male", "Female")) +
  scale_shape_manual(values = c(10, 20), 
                     name = "Sex", 
                     labels = c("Male", "Female")) +
  scale_linetype_manual(values = c("dotted", "dashed"), 
                     ...
                     labels = c("Male", "Female")) +
  facet_wrap(~country) +
  ylab("Rating (out of 1000") + xlab("Time (years)") +
  theme_minimal() +
